...
def extract_personal_info(text: str) -> Dict[str, Optional[str]]:
    ...
    m = re.search(r"(?im)^\s*(address|location|current\s*address|residence)\s*[:\-]\s*(.+)$", text)
    if m:
    ...
    # Priority 2: Look in header (first 3 lines) for "City, State" or "City, State ZIP" patterns
    ...
    # Priority 3: Full street address pattern (123 Main St, City, State ZIP)
    if not info["address"]:
        # Match complete US address: street + city + state + optional ZIP
        m = re.search(
            r'\b\d{1,5}\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,4}\s+(?:Street|St\.?|Avenue|Ave\.?|Road|Rd\.?|Boulevard|Blvd\.?|Lane|Ln\.?|Drive|Dr\.?|Court|Ct\.?|Way|Circle|Cir\.?),?\s+' +
    ...
    # Priority 4: City, State ZIP pattern (e.g., "Boston, MA 02101" or "San Francisco, CA 94102")
    if not info["address"]:
    ...
    # Priority 5: City, State pattern without ZIP (e.g., "Waltham, MA" or "Boston, Massachusetts")
    if not info["address"]:
